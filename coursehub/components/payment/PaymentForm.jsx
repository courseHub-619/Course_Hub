...
export default function PaymentForm() {
    ...
    if(!error) {
        try {
            ...
            if(response.data.success) {
                ...
                setSuccess(true)
                axios.post("http://localhost:4200/api/getOldBalence",{email:amountData.email,})
                .then(response => {
                  let newWallet = response.data.wallet+amountData.amount
                  axios.post(`http://localhost:4200/api/setNewBalence`,{
    ...
    return (
        ...
        >
          ...
          <div className="mb-3">
            <label className="font-bold text-sm mb-2 ml-1">Email<p>(please use the same email address you created this account with)</p></label>
            <div>
              <input
          ...
          <div className="mb-3">
            <label className="font-bold text-sm mb-2 ml-1">Card infos</label>
            <div className="FormRow">
            <CardElement options={CARD_OPTIONS} className="w-full px-3 py-2 mb-1 border-2 border-gray-200 rounded-md focus:outline-none focus:border-indigo-500 transition-colors"/>
          ...
          <div>
            <button className="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold">
              <i className="mdi mdi-lock-outline mr-1"></i> Pay {amountData.amount}$
            </button>
          </div>
        ...
        :
        <div className="max-w-2xl flex items-center h-auto  flex-wrap mx-auto my-36 lg:my-0 mt-96">
              <div
          className="w-full mx-auto rounded-lg bg-white shadow-lg p-5 text-gray-700"
          // style="max-width: 600px"
        >
          ...
          <div className="mb-10">
           ...
           <h1 className="text-center text-purple-600 capitalize truncate">You just got {amountData.amount} points you can start spending </h1>
       </div> 
       </div>
        }
