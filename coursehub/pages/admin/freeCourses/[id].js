...
export const getStaticProps = async (context) => {
    ...
    const teacher = await fetch(`http://localhost:4200/admin/freecourse/teacher/${id}`);    //hardcoded
    const TeacherName = await teacher.json();

    const att_id = context.params.id;
    console.log(att_id, "attachement id");
    const response = await fetch(`http://localhost:4200/admin/freecourse/attachement/${att_id}`);
...
const Course = ({ course, attachement, teacher }) => {
    ...
    const Update = () => {
        axios.put(`http://localhost:4200/admin/freeCourse/update/${course.freeCourse_id}`)
            .then(result => {
                console.log(result)
                setStatus("Accepted")
            })
    ...
    const Delete = () => {
        axios.delete(`http://localhost:4200/admin/freeCourse/delete/${course.freeCourse_id}`)
            .then(result => {
                console.log(result)
        ...
        <div className="max-w-screen-lg mx-auto">
            <main className="mt-10 p-2">
                <div className="mb-4 md:mb-0 w-full mx-auto relative">
                    ...
                    <p
                        className="py-2 text-black text-xl inline-flex items-center justify-center mb-2"
                    >
                        status: {status}
                    </p>
                    <Image
