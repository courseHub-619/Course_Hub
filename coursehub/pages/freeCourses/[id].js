...
export const getStaticPaths = async () => {
    const response = await fetch(
        'http://localhost:4200/freecourse/all'
    )
...
export const getStaticProps = async (context) => {
    ...
    const response = await fetch(`http://localhost:4200/freecourse/attachement/${att_id}`)
        ...
        <div className="max-w-screen-lg mx-auto">
            <main className="mt-10 p-2">
                ...
                <div className="mb-4 md:mb-0 w-full mx-auto relative">
                    ...
                    <Image src={course.image} width={1200} height={400} alt={""} className="w-full object-cover lg:rounded" />
                </div>
                    ...
                    <div className="w-full lg:w-1/4 m-auto mt-12 max-w-screen-sm text-center">
                        <div className="p-4 border-t border-b md:border md:rounded text-center">
                            <div className="text-center">
                                <Image className="h-14 w-10 rounded-full mr-2 object-cover" src={teacher.image} width={160} height={140} alt={""} />
                            </div>
                            <div className="text-center ">
                                <p className="font-semibold text-gray-700 text-m"> {teacher.userName} </p>
                                <p className="font-semibold text-gray-600 text-s"> Teacher </p>
                                <div className=" flex justify-center">
                                    <p className="font-semibold text-gray-600 text-xs p-2 " >Overall rating:</p>
                                    <ReactStars
                                        edit={false}
                                        count={5}
                                        size={20}
                ...
                <button class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold p-2 rounded inline-flex items-center">
                    ...
                    <span><a href={attachement.attachement}>Download attachement</a></span>
                </button>
            </main>
        </div >
    );
