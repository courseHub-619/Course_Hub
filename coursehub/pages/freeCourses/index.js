...
export async function getStaticProps() {
    ...
    const teacher = await fetch(
        'http://localhost:4200/freecourse/all/teacher'
    )
    ...
    return (
        <div className="p-6 gap-2">
            <div className="text-center" style={{ "color": "#3881AB" }}>
                <p className="py-8 text-center text-3xl font-serif font-medium" >Free Courses </p>
            </div>
                    ...
                    props.teacherList.map(teacher => {
                        return props.data.map((course) => {
                            if (course.teacher === teacher.teacher_id) {
                                return <Link href={`/freeCourses/${course.freeCourse_id}`} key={course.freeCourse_id}><Card style={{ "width": "300px", "height": "370px" }} className="text-center ">
                                    ...
                                    <div style={{ "color": "#3881AB" }}>
                                        <button className="text-center  text-l font-serif font-medium" >{course.category}</button>
                                    </div>
                                </Card>
                                </Link>
                            }
